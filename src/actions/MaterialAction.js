...
export const addMaterial = (name, material, tags) => (dispatch, getState) => {
    const { defaultTag, defaultPreview,defaultTextureSize } = getState().MaterialReducer
    dispatch({
        type: ADD_MATERIAL,
        payload: { material: material, textureSize:defaultTextureSize, preview: defaultPreview, tags: tags ? tags.concat(defaultTag) : defaultTag },
        name: name
    })
}
